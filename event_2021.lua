--[[ Optimization ]]--
local str_sub, str_match, str_gsub, str_gmatch, str_format = string.sub, string.match, string.gsub, string.gmatch, string.format
...
local module = {
	...
	prize = {
		...
		orb = { "orb_muertos", 2^1 },
		title = { "titre_muertos", 2^2 },
		pet = "familier_muertos"
	},
	time = 1.85 * 60,
	...
	confetti_to_fix_pinata = 72,
	candies_to_finish_event = 42,
	time_to_teleport_candy = 14 * 1000,
	total_guitars_in_simon_says = 14
}

if (not (tfm.get.room.uniquePlayers == 1 and tfm.get.room.playerList[module.team[1]]) and tfm.get.room.uniquePlayers < 4) or tfm.get.room.isTribeHouse then
	return system.exit()
end
...
local consumableLocations = {
	-- x1, x2, y1, y2, id
	[1] = { 80, 505, 350, 575, { "leaves", "postcard", "leaves", "brush", "leaves" } }, -- Tree
	[2] = { 1365, 430, 1560, 540, { "brush", "pinata", "postcard", "brush", "postcard", "brush" } }, -- Birb
	[3] = { 1755, 130, 2190, 290, { "microphone", "brush", "microphone", "brush", "pinata" } }, -- Bridge
	[4] = { 1890, 220, 2015, 280, { "pinata", "pumpkin" } }, -- Unreachable Platform
}
...
do
	...
	tfm.exec.giveConsumables = function(playerName, id, quantity)
		if id == consumables.pinata then
			for i = 1, quantity do
				system.giveEventGift(playerName, module.prize.pet)
			end
		else
...
local checkForOrb = function(playerName, save)
	if not hasConcluded(playerName, "orb") and playerData:get(playerName, "fixingPinata") and playerData:get(playerName, "collected") >= module.confetti_to_fix_pinata then
		system.giveEventGift(playerName, module.prize.orb[1])
		playerData:set(playerName, "conclusion", playerData:get(playerName, "conclusion") + module.prize.orb[2])
		...
		tfm.exec.giveConsumables(playerName, consumables.pinata, 5)
		tfm.exec.giveConsumables(playerName, consumables.pumpkin, 5)
...
local checkForBadge = function(playerName, save)
	if not hasConcluded(playerName, "badge") and playerData:get(playerName, "collected") >= module.candies_to_finish_event then
		...
		tfm.exec.giveConsumables(playerName, consumables.pinata, 8)
		tfm.exec.giveConsumables(playerName, consumables.pumpkin, 2)
	end
end

local checkForTitle = function(playerName)
	if not hasConcluded(playerName, "title") then
		system.giveEventGift(playerName, module.prize.title[1])
		playerData:set(playerName, "conclusion", playerData:get(playerName, "conclusion") + module.prize.title[2]):save(playerName)
...
local teleportCandies = function(playerName)
	...
	for i = 1, #playerCache[playerName].candies do
		...
		if v.time >= 0 and time() > v.time then
			...
			if not playerCache[playerName].collectedItems[i] then
				...
				v.pos = newPos
				v.time = time() + math.random(module.time_to_teleport_candy - 3000, module.time_to_teleport_candy + 3000)
...
local selectGuitar = function(self, playerName, id)
	...
	playerCache[playerName].guitarTimer = time() + 500
	...
	foo:setAction(function(self, playerName)
		if playerCache[playerName].dataLoaded then
			if playerData:get(playerName, "fixingPinata") then
				...
				if hasConcluded(playerName, "title") then
	...
	if playerData:get(playerName, "fixingPinata") then
		...
		for i = 1, #playerCache[playerName].candies do
			playerCache[playerName].candies[i] = {
				...
				time = time() + math.random(module.time_to_teleport_candy - 3000, module.time_to_teleport_candy + 3000)
			}
...
eventKeyboard = function(playerName, key, down, x, y)
	...
	if key == keys.spacebar or key == keys.down then
		...
		if playerCache[playerName].dialog[1] == 0 then
			if time > playerCache[playerName].collectTimer then
				playerCache[playerName].collectTimer = time + 500
			...
			-- Consumables
			if key == keys.down and time > playerCache[playerName].consumableTimer then
				for l = 1, #consumableLocations do
					if isInsideRectangle({ x = x, y = y }, consumableLocations[l][1], consumableLocations[l][2], consumableLocations[l][3], consumableLocations[l][4]) then
						if math.random(20) == math.random(20) then
							playerCache[playerName].consumableTimer = time + 3000
							tfm.exec.giveConsumables(playerName, consumables[table.random(consumableLocations[l][5])], math.random(1, 2))
						end
						return
					end
				end
